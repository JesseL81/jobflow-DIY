...
export default function LoginPage() {
  ...
  // Upgraded state to handle 3 views: login, signup, reset
      ...
      <Card className="w-full max-w-md bg-slate-800 border-slate-700 text-white shadow-xl rounded-2xl">
        <CardHeader className="space-y-4 text-center pb-6">
          ...
          <CardDescription className="text-slate-400 text-sm font-medium">
            {view === "signup" ? "Create your secure account" 
             : view === "reset" ? "Enter your email to receive a reset link" 
             : "Sign in to access your job site data"}
          </CardDescription>
        </CardHeader>

        <CardContent>
          
          {/* Segmented Toggle Switch (Hidden during password reset to keep UI clean) */}
          {view !== "reset" && (
          ...
          <form onSubmit={handleAuth} className="space-y-4">
              ...
              <div className="space-y-1.5 text-left">
                <div className="flex justify-between items-center">
                  ...
                  {view === "login" && (
                    <button
                      type="button"
                      onClick={() => { setView("reset"); setErrorMessage(""); setSuccessMessage(""); }}
            ...
            >
              {loading ? "Processing..." : view === "signup" ? "Create Account" : view === "reset" ? "Send Reset Link" : "Sign In"}
            </Button>
